...
export function TiempoEntrevistaConsultivaGestion() {
  ...
  const {
    ...
    queryKey: informesKeys.pv.tiempoEntrevista(JSON.stringify(filtrosAplicados)),
    queryFn: () =>
  ...
  const {
    ...
    queryKey: informesKeys.pv.tiempoEntrevista(
      JSON.stringify({ ...filtrosAplicados, bodegaDetalle, detalle: true }),
    ),
    queryFn: () =>
  ...
  return (
    <InformesPageFrame
      title={INFORMES_COPY.tiempoEntrevista.title}
      description={INFORMES_COPY.tiempoEntrevista.description}
