...
export function Retencion72Gestion() {
  ...
  const filtroByCQuery = useQuery({
    queryKey: informesKeys.pv.retencion72(`filtro:byc:${filtroByC}`),
  ...
  const familiasQuery = useQuery({
    queryKey: informesKeys.pv.retencion72(`familias:${segmentoFamilia}`),
  ...
  const filtroFamiliaQuery = useQuery({
    queryKey: informesKeys.pv.retencion72(`familia:${familiasKey.join(',')}`),
    queryFn: () =>
  ...
  const comparacionQuery = useQuery({
    queryKey: informesKeys.pv.retencion72(
      `comparacion:${modoComparacion}:${filtroComparacion}`,
    ),
    queryFn: () =>
  ...
  const vehiculos12Query = useQuery({
    queryKey: informesKeys.pv.retencion72('vehiculos:12m'),
  ...
  const vehiculosAnioQuery = useQuery({
    queryKey: informesKeys.pv.retencion72('vehiculos:anio'),
  ...
  return (
    <InformesPageFrame
      title={INFORMES_COPY.retencion72.title}
      description={INFORMES_COPY.retencion72.description}
