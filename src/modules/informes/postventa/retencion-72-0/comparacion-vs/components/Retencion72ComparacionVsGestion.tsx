...
export function Retencion72ComparacionVsGestion() {
  ...
  const generalVsQuery = useQuery({
    queryKey: informesKeys.pv.retencion72('vs:general'),
    queryFn: () => retencion720Service.obtenerVsGeneral(),
    staleTime: 60 * 1000,
  });

  const autosByCVsQuery = useQuery({
    queryKey: informesKeys.pv.retencion72(`vs:autos-byc:${filtro}`),
  ...
  return (
    <InformesPageFrame
      title="Comparación Vs - Retención 72-0"
      description="Equivalente a grafGeneralVs y GrafAutosyByCVs."
