...
export function ChecklistCarroGestion() {
  ...
  const { data: queryData, isFetching: loading, isError, error } = useQuery<{
    ...
    queryKey: informesKeys.gh.checklistCarro(
  ...
  return (
    <InformesPageFrame
      title={INFORMES_COPY.checklistCarro.title}
      description={INFORMES_COPY.checklistCarro.description}
      backHref="/dashboard/informes/gestion-humana"
      backLabel={INFORMES_COPY.backGh}
    >
      {isError ? (
        <InformesQueryError
          message={getErrorMessage(error, INFORMES_COPY.checklistCarro.loadError)}
        />
      ) : null}
