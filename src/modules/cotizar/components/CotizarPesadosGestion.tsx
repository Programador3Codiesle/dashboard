...
import { useMantenimientoPesados, usePesadosInfoClient } from "@/modules/cotizador/hooks/useCotizadorPesados";
...
export function CotizarPesadosGestion() {
  ...
  const { mantenimiento, loading: loadingMtto, refetch: refetchMtto } = useMantenimientoPesados({
    clase: info?.vehiculo?.clase ?? null,
  ...
  const totalPorGrupo = useMemo(() => {
    ...
    (mantenimiento?.grupos ?? []).forEach((g) => {
      const totalRepuestos = g.repuestos.reduce((acc, r) => acc + r.valor, 0);
      const totalManoObra = g.manoObra.reduce((acc, m) => acc + m.valor, 0);
      const totalHoras = g.manoObra.reduce((acc, m) => acc + m.horas, 0);
  ...
  const guardarMutation = useMutation({
    mutationFn: async () => {
      ...
      const grupo = mantenimiento.grupos.find((g) => g.grupo === grupoSeleccionado);
      if (!grupo) {
      ...
      const repuestos = (grupo.repuestos ?? []).map((r) => ({
        codigo: r.codigo,
      ...
      const manoObra = (grupo.manoObra ?? []).map((m) => ({
        mtto: m.operacion,
        valor: m.valor,
        estado: 1,
    ...
    onError: (error: unknown) => {
      showError(getErrorMessage(error, "No se pudo guardar la cotización de pesados."));
    },
  });
  ...
  return (
    <div className="space-y-6">
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
            ...
            <div>
              ...
              <p className="font-medium text-gray-900">{info.vehiculo?.placa}</p>
            </div>
            <div>
              <p className="text-gray-500">Clase</p>
              <p className="font-medium text-gray-900">
                {info.vehiculo?.clase} - {info.vehiculo?.descripcion}
              </p>
            </div>
            <div>
              <p className="text-gray-500">Modelo</p>
              <p className="font-medium text-gray-900">
                {info.vehiculo?.des_modelo} ({info.vehiculo?.year})
              </p>
            </div>
            <div>
              <p className="text-gray-500">Kilometraje actual</p>
              <p className="font-medium text-gray-900">
                {new Intl.NumberFormat("es-CO").format(info.vehiculo?.kilometraje)} km
              </p>
            </div>
            <div>
              <p className="text-gray-500">Kilometraje estimado</p>
              <p className="font-medium text-gray-900">
                {info.vehiculo?.km_estimado != null
                  ? `${new Intl.NumberFormat("es-CO").format(info.vehiculo?.km_estimado)} km`
                  : "N/A"}
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-4 border-t border-gray-100">
            <div>
              ...
              <input
                ...
                placeholder={info.vehiculo?.celular || "Celular"}
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
              ...
              <input
                ...
                placeholder={info.vehiculo?.mail || "correo@cliente.com"}
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
              ...
              <input
                ...
                placeholder={String(info.vehiculo?.kilometraje)}
              />
            </div>
          </div>
      ...
      {mantenimiento && (mantenimiento.grupos ?? []).length > 0 && (
        <motion.div
          initial={{ opacity: 0, y: 10 }}
          animate={{ opacity: 1, y: 0 }}
                  ...
                  <div className="flex items-center justify-between">
                    <label className="inline-flex items-center gap-2 text-sm text-gray-700">
                      <input
                        ...
                        onChange={() => {
                          if (g.grupo === "ACDelco" || g.grupo === "GM") {
                            setGrupoSeleccionado(g.grupo);
                          }
                        }}
                      />
