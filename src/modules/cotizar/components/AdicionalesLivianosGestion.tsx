...
import { ADICIONALES_LIVIANOS_SUBMENU_ID } from '@/utils/constants';
...
export function AdicionalesLivianosGestion() {
  const { blocked } = useCotizarPageGuard(ADICIONALES_LIVIANOS_SUBMENU_ID);
  ...
  const handleCrear = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: getErrorMessage(error, "No se pudo crear el adicional."),
        variant: "destructive",
      });
    }
  };
  ...
  const handleCargar = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: getErrorMessage(error, "No se pudo guardar el adicional."),
        variant: "destructive",
      });
    }
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: getErrorMessage(error, "No se pudo completar la eliminación."),
        variant: "destructive",
      });
    } finally {
      setDeleteConfirm({ open: false, type: null });
  ...
  const clases = useMemo(() => init?.clases ?? [], [init?.clases]);
