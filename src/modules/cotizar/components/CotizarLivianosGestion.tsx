...
import { COTIZAR_LIVIANOS_SUBMENU_ID } from '@/utils/constants';
...
import {
  ...
  type AdicionalesModalResponse,
  type ManoObraAdicionalModal,
  type ManoObraCotizacionRow,
  type RepuestoCotizacionRow,
} from "@/modules/cotizador/services/cotizador-livianos.service";
...
export function CotizarLivianosGestion() {
  ...
  const [repuestosState, setRepuestosState] = useState<RepuestoCotizacionRow[]>([]);
  const [manoObraState, setManoObraState] = useState<ManoObraCotizacionRow[]>([]);
  ...
  useEffect(() => {
    ...
    const repuestos = (detalle.repuestos ?? []).map((r) => {
      const obligatorio =
    ...
    const manoObra = (detalle.manoObra ?? []).map((m) => ({
      ...m,
      autorizado: true,
    }));
  ...
  const bodegasOptions = useMemo(() => {
    ...
    const filtered = raw.filter((b) => [1, 6, 7, 8].includes(Number(b.bodega)));
  ...
  const totalManoObra = useMemo(
    () =>
      (manoObraState ?? [])
        .filter((m) => m.autorizado)
        .reduce((acc, m) => acc + m.valor_unitario, 0),
    [manoObraState]
  );
  ...
  const handleAgregarAdicionalClick = async () => {
    ...
    try {
      ...
      showError(getErrorMessage(e, "No se pudieron cargar los datos del adicional."));
    } finally {
  ...
  const vehiculoYear = vehiculo?.year ?? yearActual;
  const usarValorMenos5 = yearActual - vehiculoYear <= 5;

  const handleAgregarManoObraAdicional = (mo: ManoObraAdicionalModal) => {
  ...
  const totalModalRepuestos = useMemo(() => {
    ...
    return adicionalModalData.repuestos.reduce((acc, r) => {
  ...
  const totalModalManoObra = useMemo(() => {
    ...
    return adicionalModalData.manoObra.reduce((acc, mo) => {
  ...
  const handleAgregarAdicionalSeleccionados = () => {
    ...
    const nuevosRepuestos = (adicionalModalData.repuestos ?? []).filter((r) =>
      adicionalRepuestosSeleccionados[String(r.codigo)],
    );
    const nuevosManos = (adicionalModalData.manoObra ?? []).filter((m) => {
    ...
    setRepuestosState((prev) => {
      const baseSeq =
        prev && prev.length && prev[prev.length - 1]?.seq != null
          ...
          : 0;
      let offset = 1;
      const agregados = nuevosRepuestos.map((r) => {
    ...
    setManoObraState((prev) => {
      const agregados = nuevosManos.map((mo) => {
        const precioBase = usarValorMenos5
          ? Number(mo.valor_menos_5anos ?? 0)
          : Number(mo.valor_mas_5anos ?? 0);
        const desc = Number(mo.descuento ?? 0) / 100;
  ...
  const guardarMutation = useMutation({
    mutationFn: async () => {
      ...
      const manoObra = (manoObraState ?? []).map((m) => ({
        mtto: m.operacion,
    ...
    onError: (error: unknown) => {
      showError(getErrorMessage(error, "No se pudo guardar la cotización."));
    },
  });
        ...
        >
          ...
          <div className="app-form-grid-3 text-sm">
            ...
            <div>
              ...
              <p className="font-medium text-gray-900">{vehiculo.nit}</p>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="font-medium text-gray-900">
                {vehiculo.clase} - {vehiculo.descripcion}
              </p>
            </div>
            <div>
              <p className="text-gray-500">Modelo</p>
              <p className="font-medium text-gray-900">
                {vehiculo.des_modelo} ({vehiculo.year})
              </p>
            </div>
            <div>
              <p className="text-gray-500">Kilometraje actual</p>
              <p className="font-medium text-gray-900">
                {new Intl.NumberFormat("es-CO").format(vehiculo.kilometraje)} km
              </p>
            </div>
            <div>
              <p className="text-gray-500">Kilometraje estimado</p>
              <p className="font-medium text-gray-900">
                {vehiculo.km_estimado != null
                  ? `${new Intl.NumberFormat("es-CO").format(vehiculo.km_estimado)} km`
                  : "N/A"}
              </p>
            </div>
          </div>
          ...
          <div className="app-form-grid-2 pt-4 border-t border-gray-100">
            ...
            <div>
              ...
              <select
                ...
                disabled={loadingRevisiones || !revisiones?.length}
              >
                <option value="">
              ...
              <div className="space-y-4">
                ...
                <div className="app-table-scroll">
                  <table className="w-full min-w-[640px] text-sm border border-gray-200 rounded-lg overflow-hidden">
                    ...
                    <tbody>
                      {adicionalModalData.manoObra.map((mo) => {
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="app-table-scroll">
                      <table className="w-full min-w-[640px] text-sm border border-gray-200 rounded-lg overflow-hidden">
                        ...
                        <tbody>
                          {adicionalModalData.repuestos.map((r) => {
                            const precio = Number(r.Valor ?? r.valor ?? 0);
                            const desc = Number(r.descuento ?? 0) / 100;
                            const precioConDesc = Math.round(precio * (1 - desc));
                            const key = String(r.codigo);
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    showError(getErrorMessage(e, "No se pudo crear el posible retorno."));
                  } finally {
