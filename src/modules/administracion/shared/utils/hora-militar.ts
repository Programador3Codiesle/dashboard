...
/** PHP footer_h_e: minTime '5', maxTime '6:00pm'. */
export const HORAS_EXTRA_INI_MIN = '05:00';
export const HORAS_EXTRA_INI_MAX = '18:00';

/** PHP footer_h_e: minTime '5', maxTime '11:00pm'. */
...
export function opcionesHoraMilitar(
  minHHmm: string,
  maxHHmm: string,
  ...
  const out: string[] = [];
  for (let t = from; t <= to; t += intervalo) {
    out.push(minutosAHora(t));
  }
  return out;
}

export function diferenciaHorasDecimal(horaIni: string, horaFin: string): number {
...
export const OPCIONES_HORA_AUSENTISMO = opcionesHoraMilitar(
  AUSENTISMO_HORA_MIN,
  AUSENTISMO_HORA_MAX,
);
...
export const OPCIONES_HORA_EXTRA_FIN = opcionesHoraMilitar(
  HORAS_EXTRA_FIN_MIN,
  HORAS_EXTRA_FIN_MAX,
);
