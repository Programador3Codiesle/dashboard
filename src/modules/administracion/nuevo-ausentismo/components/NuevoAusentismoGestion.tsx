...
export function NuevoAusentismoGestion() {
  ...
  const [detalleAbierto, setDetalleAbierto] =
    useState<AusentismoCalendario | null>(null);

  const query = useQuery({
    queryKey: administracionKeys.nuevoAusentismo(anioActual, mesActual),
    queryFn: () =>
      nuevoAusentismoService.obtenerCalendario(mesActual, anioActual),
    enabled: sesionLista,
    ...transactionalQueryOptions,
  });

  const saveMutation = useMutation({
    mutationFn: (payload: NuevoAusentismoDTO) =>
      nuevoAusentismoService.crearAusentismo(payload),
    onSuccess: async () => {
  ...
  const ausentismos = useMemo(() => query.data ?? [], [query.data]);
  const getAusentismosForDate = useCallback(
    (date: string) => ausentismos.filter((a) => a.fecha === date),
    [ausentismos],
  );
  ...
  return (
    <AdministracionPageFrame
      title={ADMINISTRACION_COPY.nuevoAusentismo.title}
      description={ADMINISTRACION_COPY.nuevoAusentismo.description}
    >
      {query.isError ? (
        <AdministracionQueryError
          message={getErrorMessage(
            query.error,
            ADMINISTRACION_COPY.nuevoAusentismo.loadError,
          )}
        />
      ) : null}
