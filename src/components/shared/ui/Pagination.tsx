...
export const Pagination = React.memo(({ currentPage, totalPages, onChange }: PaginationProps) => {
  ...
  const handleNext = useCallback(() => {
    ...
    return (
        <div className="flex flex-wrap justify-center items-center gap-2 sm:gap-4 bg-linear-to-r from-gray-50 to-gray-100 p-2.5 sm:p-4 rounded-xl shadow-md border border-gray-200">
            {/* Botón Anterior */}
            <button
                onClick={handlePrev}
                disabled={currentPage === 1}
                className={`
                    flex items-center gap-2 px-3 sm:px-5 py-2 sm:py-2.5 rounded-lg font-medium text-sm sm:text-base
            ...
            {/* Indicador de página */}
            <div className="flex items-center gap-2 sm:gap-3 px-3 sm:px-6 py-2 sm:py-2.5 bg-white rounded-lg border border-gray-300 shadow-sm">
                <span className="text-xs sm:text-sm font-medium text-gray-600">Página</span>
                ...
                <span className="text-xs sm:text-sm font-medium text-gray-600">de</span>
